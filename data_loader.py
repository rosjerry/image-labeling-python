...
class CarDataset(Dataset):
    ...
    def _create_label_mappings(self) -> Dict[str, Dict[str, int]]:
        ...
        for attr_name, labels in label_sets.items():
            ...
            mappings[attr_name] = {label: idx for idx, label in enumerate(sorted_labels)}
                ...
                if choices:
                    ...
                    if attr_name in self.label_mapping:
                        ...
                        if label_value in self.label_mapping[attr_name]:
                            labels[attr_name] = self.label_mapping[attr_name][label_value]
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        for attr_name in ['make', 'model', 'steer_wheel']:
            if attr_name in sample['labels']:
                labels[attr_name] = torch.tensor(sample['labels'][attr_name], dtype=torch.long)
            else:
    ...
    def get_num_classes(self) -> Dict[str, int]:
        ...
        return {attr_name: len(mapping) for attr_name, mapping in self.label_mapping.items()}
...
def create_data_loaders(json_path: str, image_dir: str, batch_size: int = 32, 
    ...
    train_dataset = CarDataset(json_path, image_dir, transform=train_transform, 
                              label_mapping=label_mapping)
    train_dataset.samples = [full_dataset.samples[i] for i in train_indices]
    
    # Create validation dataset
    val_dataset = CarDataset(json_path, image_dir, transform=val_transform, 
                            label_mapping=label_mapping)
