...
class ModelTrainer:
    ...
    def train_epoch(
            ...
            with torch.no_grad():
                for attr_name in predictions.keys():
                    if attr_name in labels:
                        ...
                        correct[attr_name] += (
                            (pred_labels == labels[attr_name]).sum().item()
                        )
        ...
        accuracies = {
            attr_name: correct[attr_name] / total[attr_name]
    ...
    def validate_epoch(
        self, val_loader: DataLoader, criterion: nn.Module, epoch: int
    ) -> Tuple[float, Dict[str, float]]:
        """
    ...
    def save_checkpoint(
        ...
        if is_best:
            ...
            torch.save(
                checkpoint, os.path.join(self.save_dir, f"checkpoint_epoch_{epoch}.pth")
            )
    ...
    def plot_training_curves(self, save_path: Optional[str] = None):
        ...
        try:
            ...
            for i, (attr_name, acc_history) in enumerate(
                self.history["val_acc"].items()
            ):
                if i < 3:  # Plot up to 3 attributes
                    row, col = (i + 1) // 2, (i + 1) % 2
                    axes[row, col].plot(
                        self.history["train_acc"][attr_name], label=f"Train {attr_name}"
                    )
