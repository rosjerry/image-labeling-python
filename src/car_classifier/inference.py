...
class CarClassifierInference:
    ...
    def predict_directory(
        self, image_dir: str, extensions: List[str] = None
...
def save_predictions(predictions: Dict, output_path: str):
    ...
    for image_path, pred_dict in predictions.items():
        ...
        for attr_name, (label, confidence) in pred_dict.items():
            serializable_pred[attr_name] = {"label": label, "confidence": confidence}
...
def main():
    ...
    if args.image_path:
        ...
        for i, (image_path, pred_dict) in enumerate(
            list(predictions.items())[:sample_count]
        ):
