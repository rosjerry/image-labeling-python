...
def parse_args() -> argparse.Namespace:
    ...
    # Data arguments
    parser.add_argument(
        "--json_path", type=str, required=True, help="Path to Label Studio JSON file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dropout_rate", type=float, default=0.5, help="Dropout rate (default: 0.5)"
    )

    # Training arguments
    parser.add_argument(
        "--epochs", type=int, default=2, help="Number of epochs (default: 100)"
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
...
def main():
    ...
    print(
        f"Training history saved to: {os.path.join(args.save_dir, 'training_history.json')}"
    )
